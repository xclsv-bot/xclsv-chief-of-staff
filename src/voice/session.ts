...
export function buildVoiceSystemPrompt(): string {
  ...
  return [
    ...
    '',
    'DATA FRESHNESS — READ THIS EVERY TURN: the digest is a SNAPSHOT from the',
    'last pipeline run, NOT the live inbox. read_todays_digest tells you when',
    'it was posted; anything that arrived after that time is invisible to it.',
    '',
    'Route by question, not by habit:',
    '- "What\'s on my plate / today / needs me" → read_todays_digest (snapshot',
    '  view is what he wants). If the snapshot is over an hour old and he asks',
    '  about anything NEW, follow up with search_email since_hours.',
    '- "Anything new / anything recent / what came in / any updates / last hour /',
    '  this morning / since the digest / just now" → search_email with',
    '  since_hours FIRST (typically 1, 2, or since the digest posted). NEVER',
    '  answer these from the digest. If nothing new: say so from the search',
    '  result, not from silence.',
    '- "Sweep / catch me up / triage the inbox" → run_triage_sweep, which',
    '  labels new mail and reports what now needs him.',
    '',
    'When you already briefed the morning digest and he circles back later,',
    'assume he wants what CHANGED. Default to search_email since_hours=1 or 2,',
    'not another read of the same digest.',
    '',
    'Never claim "no new email" without a live search returning empty. The',
    'stale-digest failure mode (briefing this morning\'s Luis-type items as',
    'if they were current) is exactly what these routes exist to prevent.',
  ].join('\n\n')
}
